...
# 第２章 業務シナリオ：2025年上半期キャンペーン施策振り返り会議
...
# 第４章：ハンズオン：インタラクティブ・ダッシュボード構築手順
...
**Step 7：【応用編】目標達成度の可視化**

**目的：**
Inputテーブルを使い、ユーザーがその場で「目標値」を自由に変更できる、動的な分析環境を構築します。そして、Lookup関数を使って、その「目標」と「実績」をリアルタイムに比較し、目標達成度を直感的に可視化します。

**手順：**

1. シミュレーション用の「目標入力テーブル」を作成

* ページに「＋」ボタンから新しい要素を追加し、**Input**を選択します。

<img width="134" height="99" alt="2-22 Inputの開き方" src="https://github.com/user-attachments/assets/3d0e2617-0c55-4074-80a5-8c25907f010d" />

* 作成されたInputテーブルに**目標管理テーブル**と名付け、以下の2つの列を作成します。

**Join Key（データ型：テキスト）**

**目標CVR（データ型：数値）**

* このテーブルに、以下のように、1行だけデータを手で入力します。

**Join Key**のカラムに**connect**を入力する

**目標CVR**は**0.55**と入力します。

※目標CVRは任意で設定してください。

<img width="239" height="120" alt="2-23 Inputテーブル内容" src="https://github.com/user-attachments/assets/3fa380d2-9170-4bf6-acdd-e09df762b4df" />

2. 次に、実績値と目標値を結合するための、計算の土台を作成します。

* 新しいピボットテーブルを作成します。
そこに、目標管理テーブルと同様に**Join Key**のカラムに**connect**を新しい計算式として、こちらのピボットテーブルにも入力してください。

**値（Values）**エリアに、以下の計算式を順番に定義していきます。

**① 実績CVR: CountDistinct([Purchase Id]) / CountDistinct([Customer Id])**

**② 目標CVR: Lookup([目標管理テーブル/目標CVR], [Join Key], [目標管理テーブル/Join Key])**

【機能解説】 Lookup関数を使い、「Input Table 1」から[目標CVR]の値を参照します。両方のテーブルに共通のキーがないため、人工的に作成した"connect"という文字列をキーにして、強制的に2つの値を紐づけています。

**③ 目標達成率: [実績CVR] / [目標CVR]**

<img width="239" height="120" alt="2-23 Inputテーブル内容" src="https://github.com/user-attachments/assets/cf4bc375-e6f3-42cc-a7f8-b93050cbb2fe" />

【機能解説】 Lookup関数を使う以外に、カラムの追加方法として、Lookupという専用ページがあるため、そちらでも同様にLookUp可能です。

<img width="440" height="283" alt="2-26 Lookupのページ表示" src="https://github.com/user-attachments/assets/54e52f9d-c4af-486b-a7a1-4e6e313d2acc" />

3.次に、ゲージチャートの作成をし、目標達成度を可視化しましう。

* 今作成したピボットテーブルを親として、グラフの種類の**Gauge**を選択します。

* 左側の設定パネルで、以下の設定を行ってください。

**値（Value）: [目標達成率]**

**目標線（Target）: 値として1を入力（1 = 100%）**

**最大値(MAX):値として、1.2を入力(1.2=120%)**

書式設定: 「パーセント」に設定し、小数点以下の桁数を0にします。

✅ このステップのゴール
Inputテーブルの「目標CVR」の数値を手で書き換えるたびに、ゲージチャートの針がリアルタイムで動く、インタラクティブな目標達成度メーターが完成します。

<img width="272" height="211" alt="2-25 目標達成度の確認" src="https://github.com/user-attachments/assets/b660c870-055c-4305-b1d2-6b949ac1cd91" />
...
主要KPIサマリーと目標到達度（ゲージチャート）を配置します。右上に時系列推移グラフを配置します。

**2. 中段（セグメントごとの比較）：**

キャンペーンタイプ別 比較グラフ（棒グラフ）を並べて配置します。広告チャネル分析（バブルチャート）を配置します。

**3. 下段（定性データの深堀り）：**

AI分析モジュール（ドーナツグラフとテーブル）を配置します。

**4. 仕上げ：**

* 下のUIメニューの**Text**を使い、ダッシュボード全体に**2025年上半期 キャンペーンサマリーデータ**という、明確なタイトルを追加します。
...
* **役割：**
    分析を主な業務とはしない、ビジネス上の意思決定者や関係者。報告会の結果を迅速に把握し、全社的な視点から、次の大きな方針を決定することが目的。
* **得られるメリット：**
    報告を聞きながら、自らの手で、瞬時に「裏付け」と「新たな発見」を得られる。
* **Sigmaの機能と、会議での具体的なアクション：**
    あなたが提示したダッシュボードで、「『新商品ローンチ型』施策が好調」であることを確認した経営層が、その場で新たな疑問を口にします。
    経営層のメンバーは、あなたに依頼するのではなく、自らダッシュボード上部の**`Ask Sigma`**に**「新商品ローンチの売上を、顧客ランク別に円グラフで」**と日本語で入力します。
...
* **得られるメリット：**
    分析と実行を、シームレスに繋ぐ。会議で明らかになったインサイトを、その場で即座に、具体的なアクションのステータス更新や、担当者への指示として、データに直接記録できる。
* **Sigmaの機能と、会議での具体的なアクション：**
    会議で、「『タイアップ・コラボ型』は、転換率に課題があるため、下半期は戦略を見直す」という方針が決定しました。
    `Act`アカウントを持つ運用担当者は、ダッシュボード上の**`Input Table`**（入力テーブル）を開き、該当する施策のステータスを「実施中」から**「要改善計画策定」に、その場で更新**します。
...
* **得られるメリット：**
    全体の議論を、自分事化し、担当領域における独自の「打ち手」を発見できる。構築担当者に依存せず、自らのビジネス課題を、自らの手で、データを使って解決できる。
* **Sigmaの機能と、会議での具体的なアクション：**
    会議で、「『タイアップ・コラボ型』は、転換率が低い」という報告を聞いた、**「人気アニメ「グルログ」コラボ」**のブランドマネージャーがいました。
    彼は、あなたが共有した公式ダッシュボードを**「コピーして（Save As）」**、自分専用の分析環境を瞬時に作成します。
    そして、**Excelのピボットテーブルと同じ感覚**で、`[Campaign Name]`で自分の担当商品に絞り込み、**行**に**Q2の回答（重視点）**、**値**に**購買転換率**を設定した新しい集計表を作成。その結果、「私の担当した『グルログ』コラボは、『キャラクター』への興味だけでなく、『味』への興味から購入した層の転換率が、他のコラボ施策より2倍も高い」という、**自らの担当領域における、独自の勝ち筋**を発見します。
