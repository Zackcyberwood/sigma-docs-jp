...
> * **何ができるか**: `Pivot集計`で「チャネル別」「広告別」「日別」といった様々な切り口でデータを整理しておきます。次に、赤くハイライトされた異常値（例：全体の応募数の急減）をクリックします。すると、その数値の内訳（例：どの広告チャネルからの応募が減ったのか）が瞬時に表示されます（ドリルダウン）。さらに気になるチャネルをクリックすれば、その中のどの広告クリエイティブに問題があるのか、といった具合に、思考を止めずに次々とデータを深掘りし、原因にたどり着くことができます。
>
## **7-2 本ユースケースの横展開案**
