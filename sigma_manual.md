...
2.  ワークブックページの右側に、コメントを追加するためのオプションを持つメニューが開きます。
3.  `Capture screenshot`を選択します。
4.  表示されるモーダルで、カーソルで描画して画像に注釈を付けます。
5.  (任意) カラーピッカーを使用して線の色を変更します。
6.  (任意) 元に戻す/やり直すオプションを使用します。
7.  画像にコメントを追加します。コメントでチームメンバーをメンションするには、`@username`の規則を使用します。
8.  `Share in comment`をクリックします。
9.  Sigmaは、注釈付きの画像をワークブックの`Comments`パネルに配置し、コメントでメンションされたユーザーにメールを送信します。
